...
const Navbar = ({ onSelectCategory, onSearch }) => {
    ...
    if (value.length >= 1) {
      ...
      try {
        const response = await axios.get(
          `http://localhost:8181/api/products/search?keyword=${value}`
        );
        setSearchResults(response.data);
  ...
  //         response = await axios.get(`http://localhost:8181/api/products/search?id=${value}`);
  //       } else {
  //         // Input is not a number, search by keyword
  //         response = await axios.get(`http://localhost:8181/api/products/search?keyword=${value}`);
  //       }
